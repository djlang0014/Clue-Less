...
cardCharacterList = [
    Card("Character", "Miss Scarlet", 1),
    Card("Character", "Col. Mustard", 2),
    Card("Character", "Mrs. White", 3),
    Card("Character", "Mr. Green", 4),
    Card("Character", "Mrs. Peacock", 5),
    Card("Character", "Prof. Plum", 6),
]
cardWeaponList =[
    Card("Weapon", "Rope", 7),
    Card("Weapon", "Lead Pipe", 8),
    Card("Weapon", "Knife", 9),
    Card("Weapon", "Wrench", 10),
    Card("Weapon", "Candlestick", 11),
    Card("Weapon", "Revolver", 12),
]
cardLocationList = [
    Card("Location", "Study", 13),
    Card("Location", "Hall", 14),
    Card("Location", "Lounge", 15),
    Card("Location", "Library", 16),
    Card("Location", "Billiard", 17),
    Card("Location", "Dining", 18),
    Card("Location", "Conservatory", 19),
    Card("Location", "Ballroom", 20),
    Card("Location", "Kitchen", 21),
]
...
def init_game():
    ...
    ejemplo2 = gameInstance.playersDict
...
def accusation(data):
    ...
    accusationCards_encoded = [card.encode('utf-8').decode('utf-8') for card in accusationCards]
    sessionCaseFile_encoded = [card.encode('utf-8').decode('utf-8') for card in sessionCaseFile]

    delim = ", "

    accusationString = "" + accuseWeapon + ", " + accuseSuspect + ", " + accuseRoom + "."

    if all(card in sessionCaseFile_encoded for card in accusationCards_encoded): 
